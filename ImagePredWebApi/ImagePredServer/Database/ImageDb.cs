...
namespace ImagePredServer.Database 
{
    public class ImageDb: IImageDb
    {
        ...
        public ClassifiedImage[] GetImageClass(int imgClass)
        {
            using (ImageDbContext dbContext=new ImageDbContext())
            {
                List<ClassifiedDbImage> dbImages = dbContext.ClassifiedImages
                    .Where(img => img.Class==imgClass).ToList();
                List<ClassifiedImage> images=new List<ClassifiedImage>();    
                foreach (var dbImage in dbImages)
                {
                    dbContext.Entry(dbImage).Reference(img => img.Image).Load();   
                    images.Add(FromDbImage(dbImage));
                }
